...
static u8 *JudgeStr(u16 waittime)
{
    ...
    {
        delay_ms(1);
        if(USART3_RX_STA & 0X8000) //接收到一次数据 15
        {
            USART3_RX_STA = 0;
...
//参数:  BufferID @ref CharBuffer1	CharBuffer2；StartPage起始页；PageNum：页数
...
u8 PS_Search(u8 BufferID, u16 StartPage, u16 PageNum, SearchResult *p)
{
    ...
    if(data)
    {
        ...
        p->pageID   = (data[10] << 8) + data[11]; //页码
        p->mathscore = (data[12] << 8) + data[13]; //得分
    }
    else
        ensure = 0xff;
    return ensure;
...
//功能:将CharBuffer1与CharBuffer2中的特征文件合并生成模板,结果存于CharBuffer1与CharBuffer2
//说明:  模块返回确认字
u8 PS_RegModel(void)
{
    u16 temp;
